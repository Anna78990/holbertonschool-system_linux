...
int print_all_symbol_tables(elf_t *elf_header, int fd, size_t *num_printed)
{
	...
	if (!size)
		headers = NULL;
	headers = calloc(size, read_size * size);
	if (!headers)
		exit(1);
	...
	char *string_table)
{
	...
	lseek(fd, SGET(i + 1, sh_offset), SEEK_SET);
	read(fd, sym_string_table, SGET(i + 1, sh_size));
	...
	{
		num_printed = print_symbol_table32(elf_header, sym_string_table, i);
	}
